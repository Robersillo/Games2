...
$result = mysqli_query($link,"select * from consejo_semana");

?>
<table class="tablanoticia">
              	    <tr>

              	      <td>Consejo de la Semana</td>

           	        </tr>
                    <?php
while ($row=mysqli_fetch_array($result))
{
echo '<tr><td>'.$row["consejo"].'</td>';
}
...
echo '<FORM METHOD="POST" ACTION="actualizado.php">ID del Consejo<br>';

//Creamos la sentencia SQL y la ejecutamos
$sSQL='Select id_consejos From consejo_semana Order By id_consejos';
$result=mysqli_query($link,$sSQL);

echo '<select name="id_consejos">';

//Generamos el menu desplegable
while ($row=mysqli_fetch_array($result))
{echo '<option>'.$row["id_consejos"];}
?>
</select>
                  </center>
</p>
<center>
              	    ...
              	    <table width="371" border="0">
              	      <tr>
              	        <td>Nuevo Consejo</td>
						<tr>
              	        <td><textarea name="consejo" id="consejo" cols="45" rows="5" placeholder=""><?php include('conex.php');$result = mysqli_query($link,"select * from consejo_semana");?><?php while ($row=mysqli_fetch_array($result)){echo $row["consejo"];}mysqli_free_result($result);?></textarea></td>
           	          </tr>
           	        </table>
                    <center>
              	    <table width="81" border="0">
              	      <tr>
              	        <td width="71"><span class="form_row">
              	         <center>
                         <input type="submit" class="register" value="Modificar" />
                         </center>
              	        </span></td>
           	          </tr>
           	        </table>
